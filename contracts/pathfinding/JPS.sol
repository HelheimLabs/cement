...
import {Coordinate as Coord} from "../utils/Coordinate.sol";
import {PQ, PriorityQueue} from "../utils/PQ.sol";

library JPS {
    using PQ for PriorityQueue;

    function findPath(uint8[][] memory _fieldInput, uint256 _startX, uint256 _startY, uint256 _endX, uint256 _endY)
        internal
        view
        returns (uint256[] memory path)
    {
        ...
        uint256[] memory nativePath =
            findPath(field, Coord.compose(_startX + 1, _startY + 1), Coord.compose(_endX + 1, _endY + 1));
     ...
     */
    function findPath(uint256[][] memory _field, uint256 _start, uint256 _end)
        internal
        view
        returns (uint256[] memory path)
    {
    ...
    function getPath(uint256[][] memory _source, uint256 _start, uint256 _end)
        internal
        pure
        returns (uint256[] memory path)
    {
